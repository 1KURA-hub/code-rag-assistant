...
func codeAnswerUserPrompt(question string, citations []Citation) string {
	...
	b.WriteString("\n请用中文回答。回答要中等详细，像 GPT 正常解释技术问题一样自然清楚。不要使用 Markdown 加粗符号，不要使用反引号包裹文件名、函数名、字段名或接口路径。可以使用普通中文标点、双引号、冒号和少量分段。先直接回答问题，再解释流程和设计原因，最后给代码依据。")
	return b.String()
}
