...
一个面向后端面试演示的代码仓库理解与变更影响分析项目。系统可以导入 GitHub 公开仓库，扫描代码文件，按 Go 函数/类型或通用行窗口切分代码，生成 embedding 后写入 PostgreSQL + pgvector，并支持代码问答、diff 影响分析和引用代码片段展示。
...
- 为什么保留本地 embedding fallback：方便无 API Key 环境演示；生产或简历展示时可以配置真实 embedding 模型。
